    ...
    this.on('data', (data) => {
        ...
        case 26:
            if (Wiegand._checkParity(data)) {
                this.emit('id', String(parseInt(data.slice(1, -1).join(''), 2)).padStart(10, "0"));
            } else {
                this.emit("error", "Parity check failed.");
            }
            break;
        default:
            this.emit("error", `Data length "${data.length}" is not equal to 26 bits.`);
        }
    });
}
